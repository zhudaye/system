    ...
    $.fn.tableExport = function (options) {
        ...
        this.each(function () {
            var $el = $(this),
                ...
                $rows = settings.headings ? $rows.add($el.find('tbody>tr')) : $rows,
                $rows = settings.footers ? $rows.add($el.find('tfoor>tr')) : $rows,
                thAdj = settings.headings ? $el.find('tbody>tr').length : 0,
            ...
            function checkCaption(exportButton) {
                var $caption = $el.find('caption:not(.title)');
